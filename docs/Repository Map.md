...
| 3 | `evaluate_model.py` | **AI Evaluation** | Evaluates the INT8 model on unseen drivers. Calculates RMSE, MAE, and outputs a visual line graph (`speed_evaluation.png`) comparing AI-predicted speed vs. true speed. |
| 4 | `dr_pipeline.py` | **Validation & EKF** | Core dead-reckoning mathematical baseline. Implements Naive double integration vs. full Error-State EKF + NHC + velocity-aiding. Validates the architecture's ability to hit <10% drift. |
